...
if (!dbgFlag || !dbgNoCORSFlag || NODE_ENV === envProduction) {
  app.use((req, res, next) => {
    ...
    /* console.log(`${curDate.toLocaleString('ru-RU')} Origin: ${origin} / method:
    ...
    if (allowedCors.includes(origin)) { // проверяем, что источник запроса есть среди разрешённых
      /* console.log(`${curDate.toLocaleString('ru-RU')} Request ${origin} is allowed:
    ...
    if (method === 'OPTIONS') { // Для предварительного запроса, добавляем нужные заголовки
      // console.log(`${curDate.toLocaleString('ru-RU')} Methods 4 preflights: ${requestHeaders}`);
...
// логгер запросов
if (!dbgFlag || !dbgNoLogFlag || NODE_ENV === envProduction) app.use(requestLogger);
...
if (!dbgFlag || !dbgNoLogFlag || NODE_ENV === envProduction) app.use(errorLogger); // логгер ошибок
app.use(errors());
app.use(errHandle);
app.listen(PORT, () => {
  logPassLint(`${curDate.toLocaleString('ru-RU')} App ${NODE_ENV} mode listening on port ${PORT}`, true);
});
